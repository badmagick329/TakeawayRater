...
class RatingModelTest(TestCase):
    ...
    def test_creation(self):
        """Basic creation test"""
        rating = Rating.objects.create(user=self.user, food=self.food, rating=5)
        self.assertEqual(rating.user, self.user)
        self.assertEqual(rating.food, self.food)
        self.assertEqual(rating.rating, 5)
        self.assertEqual(Rating.objects.count(), 1)
        rating2 = Rating.objects.create(user=self.user2, food=self.food, rating=3)
        ...
        rating3 = Rating.objects.create(user=self.user, food=self.food2, rating=4)
        ...
        rating4 = Rating.objects.create(user=self.user, food=self.food3, rating=1)
    ...
    def test_creation_fail(self):
        ...
        with self.assertRaises(ValidationError):
            rating = Rating.objects.create(user=self.user, food=self.food, rating=6)
        self.assertEqual(Rating.objects.count(), 0)
        with self.assertRaises(ValidationError):
            rating = Rating.objects.create(user=self.user, food=self.food, rating=0)
        self.assertEqual(Rating.objects.count(), 0)
        with self.assertRaises(ValidationError):
            rating = Rating.objects.create(user=self.user, food=self.food, rating=-1)
        self.assertEqual(Rating.objects.count(), 0)
    ...
    def test_duplicate_creation_fail(self):
        ...
        self.assertEqual(Rating.objects.count(), 1)
        with self.assertRaises(ValidationError):
            rating2 = Rating.objects.create(user=self.user, food=self.food, rating=3)
        self.assertEqual(Rating.objects.count(), 1)
