      ...
      {:else}
        <div class="requests__header">
          <h4 class="text-color-2">
            You have {linkRequestsList.length} link requests
          </h4>
        </div>
        <div class="requests__body">
          {#each linkRequestsList as request (request.id)}
            ...
            <div class="buttons">
              <button on:click={() => respondToLinkRequest(request.id, true)}
                >Accept</button
              >
              <button on:click={() => respondToLinkRequest(request.id, false)}
                >Decline</button
              >
            </div>
          {/each}
